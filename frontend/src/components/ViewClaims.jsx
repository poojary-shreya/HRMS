...
const ViewClaims = () => {
      ...
      try {
        ...
        const transformedClaims = response.data.data.map(claim => ({
          ...
          employee_name: claim.Employee ? 
            ...
            "N/A",
          requested_amount: claim.amount // Map amount to requested_amount for compatibility
        }));
  ...
  const handleStatusChange = async (employee_id, newStatus, claim) => {
    try {
      const response = await axios.put(
        ...
        {
          status: newStatus,
          purpose: claim.purpose, // Pass the purpose as required by the backend
        }
      );
  ...
  const filteredClaims = claims.filter(claim => {
    ...
    return (
      ...
      (claim.employee_name && claim.employee_name.toLowerCase().includes(searchValue)) 
    );
  });
