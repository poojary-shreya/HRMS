        ...
        <Card ref={payslipRef} sx={{ maxWidth: "95%", p: 3, mt: 4, boxShadow: 3, mx: 5 }}>
          <CardContent>
            ...
            <Box display="flex" justifyContent="space-between" flexDirection="row" mb={2}>
              ...
              <Box marginTop={4} display="flex" flexDirection="column" alignItems="center" textAlign="center" marginRight={60}>
                <Typography variant="body1">
                  {payslip.company_details.address || "N/A"}
                </Typography>
                <Typography variant="body1">
                  {payslip.company_details.branchLocation}
                </Typography>
              </Box>
            </Box>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Typography variant="h6" fontWeight="bold">
                {payslip.company_details.companyName}
              </Typography>
              <Typography variant="h7" color="textSecondary">
                {payslip.month} {payslip.year} | Created: {payslip.createdAt}
              </Typography>
            </Box>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <Box p={2} sx={{ backgroundColor: "#f0f0f0", borderRadius: 2 }}>
                  ...
                  <Typography><strong>Employee_ID:</strong> {payslip.employee_id}</Typography>
              ...
              <Grid item xs={6}>
                <Box p={2} sx={{ backgroundColor: "#f0f0f0", borderRadius: 2 }}>
                  ...
                  <Typography><strong>UAN:</strong>{payslip.pfDetails.UAN}</Typography>
                </Box>
              </Grid>
            </Grid>
              ...
              <Grid item xs={6}>
                ...
                <Grid container>
                  ...
                  <Grid item xs={3}>
                    ...
                    <Typography>₹{payslip.base_salary.toLocaleString()}</Typography>
                    <Typography>₹{payslip.hra.toLocaleString()}</Typography>
                    <Typography>₹{payslip.medical_allowance.toLocaleString()}</Typography>
                    <Typography>₹{payslip.newspaper_allowance.toLocaleString()}</Typography>
                    <Typography>₹{payslip.dress_allowance.toLocaleString()}</Typography>
                    <Typography>₹{payslip.other_allowance.toLocaleString()}</Typography>
                    <Divider sx={{ my: 1 }} />
                    <Typography fontWeight="bold">₹{payslip.gross_salary.toLocaleString()}</Typography>
                  </Grid>
                  <Grid item xs={3}>
                    <Typography fontWeight="bold">Cumulative</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.base_salary.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.hra.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.medical_allowance.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.newspaper_allowance.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.dress_allowance.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.other_allowance.cumulative.toLocaleString()}</Typography>
                    <Divider sx={{ my: 1 }} />
                    <Typography fontWeight="bold">₹{payslip.cumulative_salary_details.gross_salary.cumulative.toLocaleString()}</Typography>
                  </Grid>
                </Grid>
              </Grid>
              <Grid item xs={6}>
                ...
                <Grid container>
                  ...
                  <Grid item xs={3}>
                    <Typography fontWeight="bold">Amount</Typography>
                    <Typography>₹{payslip.pf.toLocaleString()}</Typography>
                    <Typography>₹{payslip.professional_tax.toLocaleString()}</Typography>
                    <Typography>₹{payslip.total_tax.toLocaleString()}</Typography>
                    <Typography color="white">-</Typography>
                    <Divider sx={{ my: 1 }} />
                    <Typography fontWeight="bold">₹{((payslip.pf) + (payslip.professional_tax) + (payslip.total_tax)).toLocaleString()}</Typography>
                  </Grid>
                  <Grid item xs={3}>
                    <Typography fontWeight="bold">Cumulative</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.pf.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.professional_tax.cumulative.toLocaleString()}</Typography>
                    <Typography>₹{payslip.cumulative_salary_details.total_tax.cumulative.toLocaleString()}</Typography>
                    <Typography color="white">-</Typography>
                    <Divider sx={{ my: 1 }} />
                    <Typography fontWeight="bold">₹{((payslip.cumulative_salary_details.pf.cumulative) + (payslip.cumulative_salary_details.professional_tax.cumulative) + (payslip.cumulative_salary_details.total_tax.cumulative)).toLocaleString()}</Typography>
                  </Grid>
                </Grid>
              </Grid>
            </Grid>
            ...
            <Typography variant="h6" fontWeight="bold" color="black"><strong>Rupees in words: </strong>{toWords(payslip.net_salary)} Only</Typography>
          </CardContent>
        </Card>
      )}
      {payslip && (
